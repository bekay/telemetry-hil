...
/* ── Downhole sensor suite simulation ─────────────────────────────────────
 *
 * Emits the six sensor line types alongside the radar DET stream:
 *   PRS:<ts>,<pressure_raw>      PSI,  0–15000
 *   TMP:<ts>,<temperature_raw>   °C,   0–175
 *   ROT:<ts>,<rotation_raw>      RPM,  0–3000
 *   DEP:<ts>,<depth_raw>         m,    0–5000
 *   TEN:<ts>,<tension_raw>       kN,   0–50
 *   SPD:<ts>,<speed_raw>         m/s,  0–5
 *
 * Raw values, no unit conversion on-device. Values are emitted as
 * "<int>.<centi>" using integer math only (no float printf on bare metal).
 * Internally everything is held in centi-units (value * 100).
 */

/* Nominal values in centi-units (× 100) */
#define SIM_PRESSURE_C     500000L   /* 5000.00 PSI */
#define SIM_TEMPERATURE_C    8500L   /* 85.00 °C    */
#define SIM_ROTATION_C     150000L   /* 1500.00 RPM */
#define SIM_DEPTH_C        150000L   /* 1500.00 m   */
#define SIM_TENSION_C        2500L   /* 25.00 kN    */
#define SIM_SPEED_C           100L   /* 1.00 m/s    */

/* Simple 16-bit LCG for deterministic pseudo-random variance */
static uint16_t lcg_state = 0xACE1u;

static uint16_t lcg_next(void)
{
    lcg_state = (uint16_t)(lcg_state * 25173u + 13849u);
    return lcg_state;
}

/* Symmetric variance in centi-units: ±range_c */
static int32_t sim_variance(int32_t range_c)
{
    if (range_c == 0) return 0;
    return (int32_t)(lcg_next() % (uint32_t)(2 * range_c + 1)) - range_c;
}

/* Format centi-units as "<int>.<2-digit-frac>" and send one sensor line */
static void SendSensorLine(const char *prefix, uint32_t timestamp, int32_t value_c)
{
    if (value_c < 0) value_c = 0;
    char buf[48];
    sprintf(buf, "%s:%lu,%ld.%02ld\r\n",
        prefix, timestamp,
        (long)(value_c / 100), (long)(value_c % 100));
    uartSendString(buf);
}

/* Emit the full sensor suite for the current scenario */
static void SendSensorSuite(uint32_t timestamp)
{
    int32_t prs = SIM_PRESSURE_C    + sim_variance(5000);  /* ±50.00 PSI  */
    int32_t tmp = SIM_TEMPERATURE_C + sim_variance(50);    /* ±0.50 °C    */
    int32_t rot = SIM_ROTATION_C    + sim_variance(3000);  /* ±30.00 RPM  */
    int32_t dep = SIM_DEPTH_C       + sim_variance(100);   /* ±1.00 m     */
    int32_t ten = SIM_TENSION_C     + sim_variance(50);    /* ±0.50 kN    */
    int32_t spd = SIM_SPEED_C       + sim_variance(5);     /* ±0.05 m/s   */

    switch (scenario)
    {
        case 2:  /* Noisy environment — occasional pressure/temperature spikes */
          if (pulse_count % 7 == 0)
              prs += 300000;                    /* +3000.00 PSI spike */
          if (pulse_count % 11 == 0)
              tmp += 4000;                      /* +40.00 °C spike    */
          break;

        case 3:  /* Fault — pressure pegged full-scale, temperature stuck */
          prs = 1500000L;                       /* 15000.00 PSI (pegged)   */
          tmp = SIM_TEMPERATURE_C;              /* stuck: zero variance    */
          break;

        default: /* 0/1: nominal suite */
          break;
    }

    SendSensorLine("PRS", timestamp, prs);
    SendSensorLine("TMP", timestamp, tmp);
    SendSensorLine("ROT", timestamp, rot);
    SendSensorLine("DEP", timestamp, dep);
    SendSensorLine("TEN", timestamp, ten);
    SendSensorLine("SPD", timestamp, spd);
}
...
void TIMER0_IRQHandler(void)
{
    ...
    /* Sensor suite streams in every scenario; scenario shaping (spikes,
     * pegged/stuck values) happens inside SendSensorSuite. */
    SendSensorSuite(timestamp);
}
